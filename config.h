...
static char *editbuf[] =    { "/bin/sh", "-c", "editbuffer" };
static char *compbuf[] =    {"/bin/sh",  "-c", "strings_extract.sh | dmenu -p Type -l 10 -i -w $(xdotool getactivewindow) | tr -d '\n' | xargs -I CC xdotool type 'CC'" };
static char *copybuf[] =    {"/bin/sh",  "-c", "strings_extract.sh | dmenu -p Copy -l 10 -i -w $(xdotool getactivewindow) | tr -d '\n' | xclip -i" };
static char *urlhandler[] = {"/bin/sh",  "-c", "xurls -r | sort | uniq | dmenu -p URL -l 10 -i -w $(xdotool getactivewindow) | xargs -r urlhandler" };
...
static Shortcut shortcuts[] = {
	...
	{ TERMMOD,              XK_P,   keyboard_select, {0} },
  ...
  { TERMMOD, XK_M,         externalpipe, { .v = urlhandler } },
};
